...
def component_scores_to_dataframe(scores: list[ComponentScore]) -> pd.DataFrame:
    ...
    for score in scores:
        ...
        data["robu_source_url"] = score.enrichment.get("source_url", "")
        data["robu_status"] = score.enrichment.get("status", "")
        data["robu_match_confidence"] = score.enrichment.get("match_confidence", "")
        ml_prediction = score.enrichment.get("ml_prediction", {})
        data["ml_predicted_score"] = ml_prediction.get("predicted_score", "")
        data["ml_confidence"] = ml_prediction.get("confidence", "")
        data["ml_risk_band"] = ml_prediction.get("risk_band", "")
        data["ml_influential_terms"] = ", ".join(ml_prediction.get("influential_terms", []))
